...
const char * const BITCOIN_CONF_FILENAME = "versus.conf";
const char * const BITCOIN_PID_FILENAME = "versusd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "versus" is a composite category enabling all Versus-related debug output
            if(ptrCategory->count(string("versus"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\VersusCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\VersusCore
    // Mac: ~/Library/Application Support/VersusCore
    // Unix: ~/.versuscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VersusCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/VersusCore";
#else
    // Unix
    return pathRet / ".versuscore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty versus.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override versus.conf
